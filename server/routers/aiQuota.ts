 ...
 * AI Quota Router
 * 
 * Handles AI Coach quota management:
...
import { aiQuota, aiUsageEvents, users } from "../../drizzle/schema";
...
router.post("/check", async (req: Request, res: Response) => {
  try {
    ...
    if (totalAvailable <= 0) {
      // No quota available - return 402 with bilingual error
      return res.status(402).json({
        allowed: false,
        error: {
          en: "Daily AI limit reached. Your quota resets tomorrow, or you can buy a Top-up Pack (60 minutes for $39).",
          fr: "Limite quotidienne atteinte. Votre quota se réinitialise demain, ou vous pouvez acheter un forfait Top-up (60 minutes pour 39$)."
        },
        dailyRemaining,
        topupBalance,
        buyTopupUrl: "/en/pricing#topup"
      });
    }
...
router.post("/consume", async (req: Request, res: Response) => {
  try {
    ...
    // Atomic update using transaction-like approach
