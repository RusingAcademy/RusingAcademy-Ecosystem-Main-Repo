    ...
    return (
      <div className="flex items-center justify-center h-48">
        <div className="text-center">
          ...
          <p className="mt-2 text-black text-sm">{isEn ? "Loading badges..." : "Chargement des badges..."}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {achievements.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-black">{isEn ? "No badges earned yet. Keep up the great work!" : "Pas encore de badges gagnés. Continuez le bon travail !"}</p>
          </div>
        ) : (
            ...
            {achievements.map((badge) => (
              ...
              >
                ...
                >
                  ...
                  <p className="text-sm font-semibold text-black">{badge.badgeName}</p>
              ...
              <div key={milestone.id} className="bg-white rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-black">{milestone.badgeName}</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-black mb-2">{selectedBadge.badgeName}</h2>
              <div
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <div className="p-6 border-b border-gray-200">
        <h3 className="text-xl font-semibold text-black">{isEn ? "Admin Leaderboard" : "Classement des administrateurs"}</h3>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-white border-b border-gray-200">
            <tr>
              ...
              <th className="text-center py-3 px-4 font-semibold text-black">{isEn ? "Reviews" : "Examens"}</th>
              <th className="text-center py-3 px-4 font-semibold text-black">{isEn ? "Badges" : "Badges"}</th>
              <th className="text-left py-3 px-4 font-semibold text-black">{isEn ? "Top Badge" : "Meilleur badge"}</th>
            </tr>
          </thead>
          <tbody>
                  ...
                  <td className="py-4 px-4">
                    {topBadge ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-black">{topBadge.badgeName}</span>
                      </div>
                    ) : (
                      <span className="text-black text-sm">{isEn ? "No badges yet" : "Pas encore de badges"}</span>
                    )}
                  </td>
                </tr>
              );
