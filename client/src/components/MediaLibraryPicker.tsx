...
function getFileIcon(mimeType: string) {
  ...
  return <File className="h-4 w-4 text-black dark:text-white dark:text-white" />;
}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 flex gap-4 overflow-hidden pb-3">
              ...
              <ScrollArea className="flex-1">
                {mediaQuery.isLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-48 text-cyan-300">
                      ...
                      >
                        {isImage(item.mimeType) ? (
                          ...
                          <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50">
                            ...
                            <span className="text-xs text-black dark:text-white dark:text-white mt-1 px-2 truncate w-full text-center">{item.fileName}</span>
                          </div>
                        )}
                        {selectedItem?.id === item.id && (
                    ...
                    {items.map((item: any) => (
                      ...
                      >
                        ...
                        <div className="flex-1 text-left">
                          ...
                          <p className="text-xs text-cyan-300">{item.mimeType} · {formatFileSize(item.fileSize)}</p>
                        </div>
                        {selectedItem?.id === item.id && (
                ...
                <div className="w-56 shrink-0 border-l pl-4 flex flex-col">
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <div>
                        <Label className="text-xs text-black dark:text-white dark:text-white">Filename</Label>
                        <p className="text-sm font-medium truncate">{selectedItem.fileName}</p>
                      </div>
                      {selectedItem.altText && (
                        <div>
                          <Label className="text-xs text-black dark:text-white dark:text-white">Alt Text</Label>
                          <p className="text-xs text-black dark:text-white dark:text-white">{selectedItem.altText}</p>
                        </div>
                      )}
                      <div className="flex gap-3">
                        <div>
                          <Label className="text-xs text-black dark:text-white dark:text-white">Type</Label>
                          <p className="text-xs">{selectedItem.mimeType}</p>
                        </div>
                        <div>
                          <Label className="text-xs text-black dark:text-white dark:text-white">Size</Label>
                          <p className="text-xs">{formatFileSize(selectedItem.fileSize)}</p>
                        </div>
                      </div>
                      {selectedItem.folder && (
                        <div>
                          <Label className="text-xs text-black dark:text-white dark:text-white">Folder</Label>
                          <Badge variant="outline" className="text-xs">{selectedItem.folder}</Badge>
                        </div>
                      )}
                      {selectedItem.tags && (
                        <div>
                          <Label className="text-xs text-black dark:text-white dark:text-white">Tags</Label>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between py-2 border-t shrink-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-black dark:text-white dark:text-white px-2">
                    {page + 1} / {totalPages}
                  </span>
                  <Button
                    size="sm"
          ...
          <TabsContent value="url" className="flex-1 flex flex-col mt-0 px-6 py-6">
            <div className="space-y-4 max-w-lg">
              ...
              {urlInput && (
                <div className="space-y-2">
                  <Label className="text-xs text-black dark:text-white dark:text-white">Preview</Label>
