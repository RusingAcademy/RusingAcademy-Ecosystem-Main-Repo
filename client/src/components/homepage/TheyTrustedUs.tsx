      ...
      <div className="container relative z-10 px-6 md:px-8 lg:px-12 lg:px-8 max-w-7xl mx-auto">
        ...
        >
          <h2 
            ...
            className="text-3xl sm:text-4xl md:text-5xl font-bold text-black dark:text-foreground mb-4 font-serif italic"
          >
            {language === 'fr' ? 'Ils nous ont fait confiance' : 'They Trusted Us'}
          </h2>
          <p className="text-base sm:text-lg text-black dark:text-foreground max-w-2xl mx-auto">
            {language === 'fr' 
          ...
          >
            {trustedOrganizations.map((org, index) => (
              ...
              >
                ...
                />
                <span 
                  className="text-[10px] sm:text-xs text-black dark:text-foreground mt-3 text-center line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 font-medium"
                  aria-hidden="true"
                >
        ...
        {/* Disclaimer */}
        <motion.p 
          className="text-center text-xs sm:text-sm text-black dark:text-foreground mt-8 italic max-w-3xl mx-auto"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
