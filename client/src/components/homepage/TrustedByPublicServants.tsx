        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {testimonials.map((testimonial, index) => (
            ...
            >
              ...
              {/* Quote */}
              <blockquote className="text-black dark:text-white dark:text-white text-sm leading-relaxed mb-6 italic min-h-[120px]">
                "{language === 'fr' ? testimonial.quote : testimonial.quoteEn}"
              </blockquote>

              {/* Author Info */}
              <div className="text-center border-t border-slate-100 pt-4">
                <p className="font-bold text-black dark:text-white dark:text-white">{testimonial.name}</p>
                <p className="text-xs text-teal-600 font-medium mt-1">
                  {language === 'fr' ? testimonial.role : testimonial.roleEn}
                </p>
                <p className="text-xs text-black dark:text-white dark:text-white mt-1">{testimonial.organization}</p>
              </div>
            </div>
          ))}
        </div>
