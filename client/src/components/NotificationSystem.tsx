...
export function NotificationBell() {
  ...
  return (
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)} className="relative p-2 text-slate-700 hover:text-slate-900 transition-colors">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border border-slate-200 z-50 max-h-96 overflow-y-auto">
          ...
          {notifications.length === 0 ? <p className="p-4 text-slate-700 text-center">No notifications</p> : notifications.slice(0, 10).map(n => {
            ...
            return (
              <div key={n.id} className={`p-4 border-b border-slate-100 hover:bg-white cursor-pointer ${!n.read ? 'bg-blue-50' : ''}`} onClick={() => markAsRead(n.id)}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1"><p className="font-medium text-slate-900 text-sm">{n.title}</p><p className="text-slate-700 text-xs mt-1">{n.message}</p></div>
                  <button onClick={(e) => { e.stopPropagation(); clearNotification(n.id); }} className="text-[#67E8F9] hover:text-slate-700 p-2 -m-2 min-h-[44px] min-w-[44px] flex items-center justify-center" aria-label="Dismiss notification"><X className="w-4 h-4" aria-hidden="true" /></button>
                </div>
              </div>
            );
          })}
