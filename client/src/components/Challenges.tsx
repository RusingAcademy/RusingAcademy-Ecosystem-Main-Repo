...
function ChallengeCard({ challenge, onSelect }: { challenge: DisplayChallenge; onSelect: (c: DisplayChallenge) => void }) {
  ...
  const statusConfig = {
    ...
    upcoming: { bg: "rgba(59, 130, 246, 0.08)", border: "rgba(59, 130, 246, 0.15)", text: "var(--color-blue-600, var(--semantic-info))", label: "Upcoming" },
    completed: { bg: "rgba(212, 175, 55, 0.08)", border: "rgba(212, 175, 55, 0.15)", text: "var(--brand-gold, var(--barholex-gold))", label: "Completed" },
  };
        ...
        {challenge.isJoined && (
          <div>
            <div className="flex items-center justify-between text-[11px] mb-1.5">
              ...
              <span className="font-bold" style={{ color: progress === 100 ? "var(--brand-gold, var(--barholex-gold))" : "var(--brand-obsidian, var(--accent-purple-deep))" }}>{progress}%</span>
            </div>
            <div className="h-2 rounded-full overflow-hidden" >
              <motion.div
                className="h-full rounded-full"
                style={{ background: progress === 100 ? "linear-gradient(90deg, var(--brand-gold, var(--barholex-gold)), #E8CB6A)" : "linear-gradient(90deg, var(--brand-obsidian, var(--accent-purple-deep)), var(--brand-obsidian, var(--accent-purple-dark)))" }}
                initial={{ width: 0 }}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      ...
      {challenge.isJoined && (
        ...
        >
          <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full opacity-10 blur-2xl" style={{ background: "linear-gradient(135deg, var(--brand-gold, var(--barholex-gold)), var(--brand-obsidian, var(--accent-purple-deep)))" }} />
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Flame className="w-5 h-5"  />
              <span className="font-bold text-foreground">Your Progress</span>
            </div>
            <span className="text-lg font-bold" style={{ color: progress === 100 ? "var(--brand-gold, var(--barholex-gold))" : "var(--brand-obsidian, var(--accent-purple-deep))" }}>{progress}%</span>
          </div>
      ...
      >
        <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, var(--brand-gold, var(--barholex-gold)), #E8CB6A)" }}>
