...
export function GamificationDashboard({ compact = false }: GamificationDashboardProps) {
  ...
  if (compact) {
    return (
      <div className="bg-white dark:bg-slate-800 dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-black dark:text-white dark:text-white">{stats.levelInfo.title}</p>
              <p className="text-sm text-black dark:text-white dark:text-white">{stats.xp.total.toLocaleString()} XP</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-black dark:text-white dark:text-white">day streak</p>
            </div>
          </div>
        </div>
        ...
        {stats.levelInfo.nextLevel && (
          <div className="mt-3">
            <div className="flex justify-between text-xs text-black dark:text-white dark:text-white mb-1">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-teal-500 to-teal-600 rounded-2xl p-6 text-white shadow-xl">
        <div className="flex items-center justify-between mb-6">
          ...
          {/* Streak */}
          <div className="text-center bg-white dark:bg-slate-800 dark:bg-slate-900/10 rounded-xl p-4 backdrop-blur">
        ...
        {stats.levelInfo.nextLevel && (
          <div>
            ...
            <div className="h-3 bg-white dark:bg-slate-800 dark:bg-slate-900/20 rounded-full overflow-hidden">
              <div 
                className="h-full bg-white dark:bg-slate-800 dark:bg-slate-900 rounded-full transition-all duration-500"
                style={{ width: `${stats.levelInfo.progressPercent}%` }}
              />
            </div>
      ...
      {/* Badges Section */}
      <div className="bg-white dark:bg-slate-800 dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-black dark:text-white dark:text-white">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
            {(showAllBadges ? allBadges : stats.badges.recent)?.map((badge) => (
              ...
              >
                ...
                <p className="text-xs font-medium text-black dark:text-white dark:text-white truncate">{badge.title}</p>
      ...
      <div className="bg-white dark:bg-slate-800 dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-black dark:text-white dark:text-white">Leaderboard</h3>
          ...
          {leaderboard?.entries?.map((entry, index) => (
            ...
            >
              ...
              <div className="flex-1">
                <p className="font-medium text-black dark:text-white dark:text-white">{entry.name || "Anonymous"}</p>
                <p className="text-sm text-black dark:text-white dark:text-white">Level {entry.level} • {entry.levelTitle}</p>
              </div>
