      ...
      >
        ...
        <div className="relative aspect-video bg-black">
          ...
          <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/90 to-transparent p-6 transition-all duration-500 ${showControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            {/* Progress Bar */}
            <div 
              className="relative h-2 bg-white dark:bg-slate-800 dark:bg-slate-900/20 rounded-full cursor-pointer mb-4 group"
              onClick={handleProgressClick}
            >
              <div className="absolute inset-0 bg-white dark:bg-slate-800 dark:bg-slate-900/10 rounded-full" />
              <div 
                className={`h-full bg-gradient-to-r ${accentColor} rounded-full relative transition-all duration-100`}
                style={{ width: `${progress}%` }}
              >
                <div className="absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4 bg-white dark:bg-slate-800 dark:bg-slate-900 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <button
                  onClick={togglePlay}
                  aria-label={isPlaying ? 'Pause video' : 'Play video'}
                  className="w-12 h-12 rounded-full bg-white dark:bg-slate-800 dark:bg-slate-900/10 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white dark:bg-slate-800 dark:bg-slate-900/20 transition-all hover:scale-110 border border-white/60"
                >
                ...
                <button
                  ...
                  className="w-10 h-10 rounded-full bg-white dark:bg-slate-800 dark:bg-slate-900/10 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white dark:bg-slate-800 dark:bg-slate-900/20 transition-all hover:scale-110 border border-white/60"
                >
...
function CoachCard({ 
  ...
  return (
    ...
    >
      ...
      />
      <div className="absolute inset-[2px] rounded-[calc(2rem-2px)] bg-white dark:bg-slate-800 dark:bg-slate-900 z-0" />
      ...
      <div className="relative aspect-[4/3] overflow-hidden z-10">
        {isPlayingInline && bunnyPlayUrl ? (
          ...
          <>
            ...
            <div className="absolute top-4 right-4 flex items-center gap-1.5 bg-white dark:bg-slate-800 dark:bg-slate-900/95 backdrop-blur-md px-3 py-1.5 rounded-full shadow-lg border border-white/70">
              <Star className="w-4 h-4 text-amber-500 fill-amber-500" />
              <span className="text-sm font-bold text-black dark:text-white dark:text-white">{coach.rating.toFixed(1)}</span>
            </div>
            ...
            >
              <div className={`relative transition-all duration-500 ${isHovering ? 'scale-110' : 'scale-100'}`}>
                ...
                {/* Play Button */}
                <div className={`relative w-16 h-16 rounded-full flex items-center justify-center shadow-2xl backdrop-blur-md transition-all duration-500 border-2 ${isHovering ? 'bg-white dark:bg-slate-800 border-teal-400 shadow-teal-500/30' : 'bg-white/90 border-white/70'}`}>
      ...
      {/* Content */}
      <div className="relative z-20 p-6 bg-white dark:bg-slate-800 dark:bg-slate-900">
        <h3 className="text-xl font-bold text-black dark:text-white dark:text-white mb-1 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors">
          {coach.name}
        </h3>
        <p className={`text-sm font-semibold bg-gradient-to-r ${coach.accentColor} bg-clip-text text-transparent mb-3`}>
          {coach.headline}
        </p>
        <p className="text-black dark:text-white dark:text-white/90 text-sm line-clamp-2 mb-4">
          {coach.bio}
        </p>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex gap-2">
            {coach.languages.map((lang) => (
              <span 
                key={lang}
                className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 dark:bg-slate-800 dark:bg-teal-800/50 rounded-full text-xs font-medium text-black dark:text-white dark:text-white/90"
              >
          ...
          {coach.availability.availableToday ? (
            ...
            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-black dark:text-white dark:text-white/90">
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-4 pt-1">
          ...
          <p className="text-sm md:text-base text-black dark:text-white dark:text-white font-medium leading-snug max-w-6xl mx-auto">
            {language === 'fr' 
