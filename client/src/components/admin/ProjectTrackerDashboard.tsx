...
const KPICard = ({ icon: Icon, label, value, subtitle, progress }: { icon: any; label: string; value: string | number; subtitle: string; progress?: number; }) => (
  <div className="bg-white dark:bg-slate-800 dark:bg-slate-900/5 backdrop-blur-sm border border-white/60 rounded-2xl p-6 hover:bg-white dark:bg-slate-800 dark:bg-slate-900/10 transition-all duration-300">
  ...
  return (
    <div className={`bg-white/5 backdrop-blur-sm rounded-xl p-4 ${path.priority ? 'border border-amber-500/50' : 'border border-white/60'}`}>
      ...
      <div className="space-y-2 text-sm">
        <div className="flex justify-between"><span className="text-cyan-300">Structure</span><span className={path.structureComplete ? 'text-emerald-400' : 'text-red-400'}>{path.structureComplete ? '✓ Complète' : '✗ Incomplète'}</span></div>
        <div className="flex justify-between"><span className="text-cyan-300">Vidéos ({path.videosTotal})</span><span className={videoProgress > 0 ? 'text-emerald-400' : 'text-red-400'}>{videoProgress.toFixed(0)}% Produit</span></div>
...
export default function ProjectTrackerDashboard() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-obsidian via-teal-900 to-obsidian flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-cyan-300">Ce tableau de bord est réservé aux administrateurs.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-obsidian via-teal-900 to-obsidian text-white">
      <header className="bg-white dark:bg-slate-800 dark:bg-slate-900/5 backdrop-blur-sm border-b border-white/60 sticky top-0 z-50 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-cyan-300">Dernière mise à jour: {data.lastUpdated}</span>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div className="lg:col-span-2 bg-white dark:bg-slate-800 dark:bg-slate-900/5 backdrop-blur-sm border border-white/60 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              {data.branches.map((branch, index) => (
                <div key={index} className="bg-white dark:bg-slate-800 dark:bg-slate-900/5 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
          ...
          <div className="bg-white dark:bg-slate-800 dark:bg-slate-900/5 backdrop-blur-sm border border-white/60 rounded-2xl p-6">
        ...
        <section className="bg-white dark:bg-slate-800 dark:bg-slate-900/5 backdrop-blur-sm border border-white/60 rounded-2xl p-6 mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <div className="bg-white dark:bg-slate-800 dark:bg-slate-900/5 backdrop-blur-sm border border-white/60 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {data.techStack.map((tech, index) => (<div key={index} className="flex items-center justify-between bg-white dark:bg-slate-800 dark:bg-slate-900/5 rounded-lg p-3"><span className="text-cyan-300">{tech.name}</span><span className="text-emerald-400">{tech.value}</span></div>))}
            </div>
          </div>
          ...
          <div className="space-y-4">
            {data.sprints.slice(0, 6).map((sprint) => (
              <div key={sprint.id} className="flex items-start gap-4">
                ...
                <div className="flex-1 bg-white dark:bg-slate-800 dark:bg-slate-900/5 rounded-xl p-4">
                  ...
                  <p className="text-sm text-cyan-300">{sprint.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="bg-white dark:bg-slate-800 dark:bg-slate-900/5 backdrop-blur-sm border border-white/60 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center"><div className="text-5xl font-bold text-amber-400">{data.mediaProduction.totalAssets}</div><p className="text-cyan-300 mt-2">Assets Média Total</p><p className="text-xs text-black dark:text-white dark:text-white">{data.mediaProduction.videosTotal} vidéos + {data.mediaProduction.audiosTotal} audios</p></div>
            <div className="text-center"><div className="text-5xl font-bold text-blue-400">{data.mediaProduction.estimatedMonths}</div><p className="text-cyan-300 mt-2">Mois de Production</p><p className="text-xs text-black dark:text-white dark:text-white">Avec équipe dédiée</p></div>
            <div className="text-center"><div className="text-4xl font-bold text-foundation">${(data.mediaProduction.estimatedBudgetMin / 1000).toFixed(0)}K-${(data.mediaProduction.estimatedBudgetMax / 1000).toFixed(0)}K</div><p className="text-cyan-300 mt-2">Budget Estimé (CAD)</p><p className="text-xs text-black dark:text-white dark:text-white">Incluant AI-assisted production</p></div>
          </div>
        </section>
      </main>

      <footer className="bg-white dark:bg-slate-800 dark:bg-slate-900/5 border-t border-white/60 mt-8 px-6 py-4">
