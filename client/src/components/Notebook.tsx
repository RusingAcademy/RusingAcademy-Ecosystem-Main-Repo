...
const statusConfig: Record<string, { icon: typeof CheckCircle2; label: string; color: string; bg: string; border: string }> = {
  corrected: { icon: CheckCircle2, label: "Corrected", color: "var(--brand-gold, var(--barholex-gold))", bg: "rgba(212, 175, 55, 0.06)", border: "rgba(212, 175, 55, 0.12)" },
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm leading-relaxed">
            ...
            <span className="font-semibold px-1 rounded" style={{ color: "var(--brand-gold, var(--barholex-gold))", background: "rgba(212, 175, 55, 0.06)" }}>{correction.corrected}</span>
          </p>
    ...
    >
      ...
      {entry.correctionCount > 0 && (
        <div className="flex items-center gap-2 mb-3 text-[11px]">
          <span className="px-2 py-0.5 rounded-lg font-bold" style={{ background: "rgba(239, 68, 68, 0.05)", color: "var(--semantic-danger, var(--danger))" }}>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      ...
      >
        ...
        <div className="absolute top-0 left-0 right-0 h-1" style={{ background: "linear-gradient(90deg, var(--brand-obsidian, var(--accent-purple-deep)), var(--brand-gold, var(--barholex-gold)))" }} />

        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center text-white font-bold text-sm" style={{ background: "linear-gradient(135deg, var(--brand-obsidian, var(--accent-purple-deep)), var(--brand-obsidian, var(--accent-purple-dark)))" }}>
              {(entry.authorName || "?").split(" ").map(n => n[0]).join("").slice(0, 2)}
            </div>
            <div>
          ...
          {corrections.map((c: any) => (
            <div key={c.id} className="rounded-xl p-4 space-y-2" style={{ background: "white", border: "1px solid rgba(27, 20, 100, 0.05)", boxShadow: "0 1px 4px rgba(27, 20, 100, 0.03)" }}>
              <div className="flex items-center gap-2 mb-2">
                <div className="w-6 h-6 rounded-lg flex items-center justify-center text-white font-bold text-[10px]" style={{ background: "linear-gradient(135deg, var(--brand-gold, var(--barholex-gold)), #E8CB6A)" }}>
                  {(c.correctorName || "?")[0]}
                </div>
      ...
      {entry.status === "pending" && !showCorrectionForm && (
        <div className="rounded-2xl p-5 text-center" style={{ background: "rgba(27, 20, 100, 0.02)", border: "1px solid rgba(27, 20, 100, 0.04)" }}>
          ...
          <Button
            ...
            style={{ background: "linear-gradient(135deg, var(--brand-gold, var(--barholex-gold)), #E8CB6A)", boxShadow: "0 2px 8px rgba(212, 175, 55, 0.2)" }}
          >
...
function WriteEntryForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      <button aria-label="Action" onClick={onClose} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors font-medium">
        <ArrowLeft className="w-4 h-4" /> Back to notebook
      </button>
        ...
        <div className="flex gap-2">
          {(["french", "english"] as const).map((lang) => (
            <button
              ...
              style={{
                background: language === lang ? "linear-gradient(135deg, var(--brand-obsidian, var(--accent-purple-deep)), var(--brand-obsidian, var(--accent-purple-dark)))" : "rgba(27, 20, 100, 0.03)",
      ...
      <div className="flex items-center gap-1.5 p-0.5 rounded-xl w-fit" style={{ background: "rgba(27, 20, 100, 0.03)" }}>
        {["all", "pending", "corrected", "perfect"].map((f) => (
          <button
            ...
            style={{
              background: filter === f ? "linear-gradient(135deg, var(--brand-obsidian, var(--accent-purple-deep)), var(--brand-obsidian, var(--accent-purple-dark)))" : "transparent",
