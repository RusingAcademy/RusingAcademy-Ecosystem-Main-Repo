    ...
    <div className={cn("relative inline-flex flex-col items-center", className)}>
      ...
      {(showValue || showLabel) && (
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          {showLabel && label && (
            <span className={cn("font-medium text-black dark:text-foreground dark:text-cyan-300", config.labelSize)}>
              {label}
            </span>
          )}
        </div>
      ...
      {/* Sublabel below ring */}
      {sublabel && (
        <span className={cn("mt-2 text-black dark:text-foreground dark:text-cyan-300", config.labelSize)}>
          {sublabel}
        </span>
      )}
    </div>
  ...
  return (
    <div className={cn("flex flex-col items-center", className)}>
      <div className="relative">
        ...
        />
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-sm text-black dark:text-foreground dark:text-cyan-300">{l.current}</span>
      ...
      <div className="mt-3 text-center">
        <p className="text-sm text-black dark:text-foreground dark:text-cyan-300">
          {l.progress} <span className="font-semibold text-black dark:text-foreground">{targetLevel}</span>
        </p>
        <p className="text-lg font-bold text-black dark:text-foreground">{percentage}%</p>
      </div>
    </div>
  );
}
      ...
      {skills.map((skill) => {
        ...
        return (
          <div key={skill.key} className="flex flex-col items-center">
            ...
            <p className="mt-2 text-sm font-medium text-black dark:text-foreground/90">{skill.label}</p>
