    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[80vh]">
        <DialogHeader>
          ...
          {sectionTitle && (
            <p className="text-xs text-black dark:text-foreground mt-1">Section: {sectionTitle}</p>
          )}
        </DialogHeader>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {(rev as any).sectionTitle && (
                            <span className="text-[10px] text-black dark:text-foreground font-medium truncate max-w-[120px]">
                              {(rev as any).sectionTitle}
                            </span>
                          )}
                        </div>
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="bg-red-50 rounded p-2 text-[10px] space-y-0.5 max-h-32 overflow-auto">
                                {Object.entries(rev.previousData).map(([k, v]) => (
                                  <div key={k}>
                                    <span className="text-black dark:text-foreground">{k}:</span>{" "}
                                    <span className="text-black dark:text-foreground break-all">{typeof v === "object" ? JSON.stringify(v) : String(v ?? "—")}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
