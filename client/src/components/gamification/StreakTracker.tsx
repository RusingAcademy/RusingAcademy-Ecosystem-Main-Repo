...
export function StreakDisplay({ streak, size = 'default' }: { streak: StreakData; size?: string }) {
  ...
  return (
    <motion.div 
      className={`flex items-center gap-2 p-3 rounded-xl ${isActive ? 'bg-orange-500/20' : 'bg-gray-500/20'}`}
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-sm">
        ...
        <span className="text-violet-400">{nextMilestone.label} ({nextMilestone.days}j)</span>
      </div>
      <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
        <motion.div className="h-full bg-gradient-to-r from-orange-500 to-red-500" initial={{ width: 0 }} animate={{ width: `${Math.min(progress, 100)}%` }} />
      </div>
