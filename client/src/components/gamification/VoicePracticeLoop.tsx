  ...
  return (
    <div className="bg-[#1A1A2E] rounded-2xl p-6 border border-white/10">
      <div className="mb-6">
        ...
        <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md/5 rounded-xl p-4">
        ...
        <AnimatePresence mode="wait">
          ...
          {state === 'countdown' && (
            <motion.div key="countdown" className="text-center">
              <div className="w-24 h-24 rounded-full bg-cta/20 flex items-center justify-center">
                <span className="text-2xl md:text-4xl lg:text-5xl font-bold text-amber-400">{countdown}</span>
              </div>
          ...
          {state === 'recording' && (
            <motion.div key="recording" className="text-center">
              ...
              <button onClick={stopRecording} className="mt-4 px-6 py-2 bg-white dark:bg-white/[0.08] dark:backdrop-blur-md/10 rounded-lg text-white">Terminer</button>
            </motion.div>
          )}
          {state === 'processing' && (
          ...
          {state === 'feedback' && feedback && (
            <motion.div key="feedback" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full">
              <div className="text-center mb-6">
                <div className={`text-3xl md:text-5xl lg:text-6xl font-bold ${feedback.overallScore >= 70 ? 'text-green-400' : 'text-amber-400'}`}>
                  {feedback.overallScore}%
                </div>
                <p className="text-white/60">{feedback.overallScore >= 70 ? 'Excellent travail!' : 'Continuez vos efforts!'}</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md/5 rounded-xl p-4 text-center">
              ...
              {feedback.suggestions.length > 0 && (
                <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md/5 rounded-xl p-4 mb-4">
              ...
              <div className="flex gap-3">
                {attempts < VOICE_CONFIG.retryLimit && feedback.overallScore < VOICE_CONFIG.passingScore && (
                  <button onClick={retry} className="flex-1 py-3 bg-white dark:bg-white/[0.08] dark:backdrop-blur-md/10 rounded-xl text-white">Réessayer</button>
                )}
