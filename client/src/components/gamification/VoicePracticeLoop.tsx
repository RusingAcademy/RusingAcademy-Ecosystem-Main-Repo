    ...
    <div className="bg-[#1A1A2E] rounded-2xl p-6 border border-white/10">
      <div className="mb-6">
        ...
        <div className="bg-white/5 rounded-xl p-4">
          ...
          {promptAudio && <button className="mt-3 flex items-center gap-2 text-violet-400 text-sm">🔊 Écouter l'exemple</button>}
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {state === 'idle' && (
            <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={startRecording}
                className="w-24 h-24 rounded-full bg-gradient-to-br from-red-500 to-pink-600 flex items-center justify-center shadow-lg">
          ...
          {state === 'processing' && (
            <motion.div key="processing" className="text-center">
              ...
              <p className="mt-4 text-violet-400">Analyse en cours...</p>
            </motion.div>
          )}
            ...
            <motion.div key="feedback" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full">
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="bg-white/5 rounded-xl p-4 text-center">
                  ...
                  <p className="text-xs text-white/40">Prononciation</p>
                </div>
                <div className="bg-white/5 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-green-400">{feedback.fluency.score}%</p>
                  <p className="text-xs text-white/40">Fluidité</p>
                </div>
                <div className="bg-white/5 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-violet-400">{feedback.grammar.score}%</p>
                  <p className="text-xs text-white/40">Grammaire</p>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => onComplete(feedback.overallScore >= VOICE_CONFIG.passingScore, feedback.overallScore)}
                  className="flex-1 py-3 bg-violet-600 rounded-xl text-white">Continuer</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
