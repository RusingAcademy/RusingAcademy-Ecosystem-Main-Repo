...
export default function Certificate({
  ...
  const handleDownload = () => {
    if (pdfUrl) {
      ...
      link.href = pdfUrl;
      link.download = `RusingÂcademy-Certificate-${certificateNumber}.pdf`;
      link.target = "_blank";
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ref={certificateRef}
        className="relative bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-lg shadow-2xl overflow-hidden"
        style={{
        ...
        <div className="relative h-full flex flex-col items-center justify-center px-12 py-8 text-center">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-black dark:text-foreground">{content.organizationTagline}</p>
            </div>
          </div>
          ...
          {/* Completion Text */}
          <p className="text-lg text-black dark:text-foreground mb-2">{content.hasCompleted}</p>

          {/* Course Title */}
          <p className="text-2xl font-semibold text-black dark:text-foreground mb-2 max-w-lg">
            {courseTitle}
          </p>

          {/* Path info */}
          {pathTitle && (
            <p className="text-sm text-black dark:text-foreground italic mb-1">
              {isEn
          ...
          <div className="flex items-end justify-between w-full max-w-2xl mt-auto">
            {/* Date */}
            <div className="text-left">
              <p className="text-sm text-black dark:text-foreground">{content.issuedOn}</p>
            ...
            <div className="text-center">
              <div className="w-48 border-b-2 border-gray-400 mb-2">
                <p className="text-2xl font-script text-black dark:text-foreground italic">
                  Prof. Steven Rusinga
                </p>
              </div>
              <p className="text-sm text-black dark:text-foreground">{content.signedBy}</p>
