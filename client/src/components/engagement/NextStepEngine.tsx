    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-8 p-6 rounded-2xl bg-gradient-to-br from-foundation/10 to-teal-700/10 border border-foundation/20">
      ...
      {primaryReco && !showAllOptions && (
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="space-y-4">
          <div className="p-4 rounded-xl bg-white dark:bg-background/5 border border-white/10">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button onClick={handleOverride} className="px-6 py-3 bg-white dark:bg-background/5 text-white/70 font-medium rounded-xl border border-white/10 hover:bg-white dark:bg-background/10 transition-all duration-300">Autres options</button>
          </div>
        </motion.div>
      )}
      {showAllOptions && (
