  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-[#1A1A2E] rounded-2xl p-6 max-w-md w-full border border-white/10">
        ...
        <div className="flex gap-3"><button onClick={onClose} className="flex-1 py-3 bg-white dark:bg-slate-800 dark:bg-slate-900/10 rounded-xl text-white">Plus tard</button><button onClick={onClose} className="flex-1 py-3 bg-foundation-soft rounded-xl text-white font-medium">Reprendre</button></div>
      </motion.div>
    </motion.div>
  );
          }
