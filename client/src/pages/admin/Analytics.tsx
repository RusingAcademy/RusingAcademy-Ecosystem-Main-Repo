...
const t = {
  en: {
    ...
    subtitle: "Track performance, engagement, and revenue across your ecosystem.",
    ...
    enrolled: "enrolled",
    noCourseData: "No course data yet",
    monthlyRevenue: "Monthly Revenue",
    noRevenueData: "No revenue data yet",
    platformHealth: "Platform Health",
    activeThisWeek: "Active This Week",
    completions: "Completions",
    coachesWithStripe: "Coaches w/ Stripe",
    coachesWithoutStripe: "Coaches w/o Stripe",
  },
  fr: {
    title: "Analytique et rapports",
    subtitle: "Suivez la performance, l'engagement et les revenus de votre écosystème.",
    totalUsers: "Utilisateurs totaux",
    activeCoaches: "Coachs actifs",
    sessions: "Sessions (mois)",
    revenue: "Revenus",
    learners: "apprenants",
    pending: "en attente",
    coursePerformance: "Performance des cours",
    enrolled: "inscrits",
    noCourseData: "Aucune donnée de cours pour le moment",
    ...
    coachesWithStripe: "Coachs avec Stripe",
    coachesWithoutStripe: "Coachs sans Stripe",
  },
};
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: ui.totalUsers, value: totalUsers, icon: Users, color: "var(--color-blue-600, #2563eb)", sub: `${totalLearners} ${ui.learners}` },
          { label: ui.activeCoaches, value: activeCoaches, icon: GraduationCap, color: "var(--semantic-success, #059669)", sub: `${pendingCoaches} ${ui.pending}` },
          { label: ui.sessions, value: sessionsThisMonth, icon: Calendar, color: "var(--color-violet-600, #7c3aed)", sub: a.sessionGrowth ? `${a.sessionGrowth > 0 ? "+" : ""}${a.sessionGrowth}%` : "" },
          { label: ui.revenue, value: `$${(revenue / 100).toLocaleString()}`, icon: DollarSign, color: "var(--semantic-danger, #dc2626)", sub: a.revenueGrowth ? `${a.revenueGrowth > 0 ? "+" : ""}${a.revenueGrowth}%` : "" },
        ].map((s) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><TrendingUp className="h-4 w-4" /> {ui.monthlyRevenue}</CardTitle></CardHeader>
          ...
          ) : <p className="text-sm text-muted-foreground text-center py-4">{ui.noRevenueData}</p>}</CardContent>
        </Card>
      </div>
      <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Activity className="h-4 w-4" /> {ui.platformHealth}</CardTitle></CardHeader>
        <CardContent><div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: ui.activeThisWeek, value: o.activeThisWeek ?? 0 },
            { label: ui.completions, value: o.completions ?? 0 },
            { label: ui.coachesWithStripe, value: a.coachesWithStripe ?? 0 },
            { label: ui.coachesWithoutStripe, value: a.coachesWithoutStripe ?? 0 },
          ].map(s => (
