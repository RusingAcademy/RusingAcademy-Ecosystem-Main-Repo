...
export default function CRMPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div><h1 className="text-2xl font-bold">CRM & Contacts</h1><p className="text-sm text-muted-foreground">Manage leads, inquiries, and customer relationships.</p></div>
      ...
      <Card><CardContent className="p-0">
        {isLoading ? <div className="p-6 space-y-3">{[1,2,3].map(i => <Skeleton key={i} className="h-12 w-full" />)}</div> : allInquiries.length === 0 ? (
          <p className="p-8 text-center text-muted-foreground">No inquiries yet</p>
        ) : (
          <div className="divide-y">{allInquiries.slice(0, 20).map((inq: any) => (
            <div key={inq.id} className="p-4 flex items-center justify-between">
              <div><p className="font-medium">{inq.name || inq.email || "Contact"}</p><p className="text-sm text-muted-foreground">{inq.message?.substring(0, 80) || "No message"}...</p><p className="text-xs text-muted-foreground mt-1">{inq.createdAt ? new Date(inq.createdAt).toLocaleDateString() : ""}</p></div>
