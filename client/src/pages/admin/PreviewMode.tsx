  ...
  if (viewMode === "select") {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-black dark:text-foreground">Preview As</h1>
                ...
                <CardContent className="pt-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-black dark:text-foreground text-lg">{mode.title}</h3>
                      <p className="text-sm text-black dark:text-foreground mt-1">{mode.description}</p>
                            ...
                            {(mode.id === "learner" ? students : coaches)?.map((u: any) => (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <span className="text-cyan-300 text-xs block truncate">{u.email}</span>
                                </div>
                                <ChevronRight className="w-4 h-4 text-white/90 shrink-0" />
                              </button>
                            )) || (
                              <p className="text-xs text-cyan-300 px-3 py-2">No users found</p>
                            )}
                          </div>
                        </div>
                      )}
        ...
        <Card className="border-dashed">
          <CardContent className="py-4">
            <div className="flex items-center gap-6 text-sm text-black dark:text-foreground">
      ...
      <div className={`p-4 rounded-xl ${currentMode?.bg} border-2 ${currentMode?.border}`}>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-black dark:text-foreground">
                Previewing as: {currentMode?.title}
                {selectedUserId && previewData && (
                  <span className="font-normal text-black dark:text-foreground">
                    {" "}— {(previewData as any).user?.name || (previewData as any).profile?.name || ""}
                  </span>
                )}
              </h2>
              <p className="text-sm text-black dark:text-foreground">Read-only preview of what this user type sees</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
      ...
      {previewTab === "data" && (
        <>
          ...
          {viewMode === "public" && previewData && (
            <div className="space-y-6">
              <div className="flex items-center gap-2 text-sm text-black dark:text-foreground bg-blue-50 p-3 rounded-lg">
                ...
                {((previewData as any).courses || []).map((c: any) => (
                  <Card key={c.id} className="hover:shadow-md transition-shadow">
                    <CardContent className="pt-5">
                      ...
                      <h4 className="font-semibold text-black dark:text-foreground">{c.title}</h4>
                      <p className="text-sm text-black dark:text-foreground mt-1 line-clamp-2">{c.description}</p>
                      <div className="flex items-center justify-between mt-3">
                        <span className="text-lg font-bold text-violet-600">
                          {c.price ? `$${(c.price / 100).toFixed(2)}` : "Free"}
                        </span>
                        <span className="text-xs text-cyan-300">{c.totalEnrollments || 0} enrolled</span>
                      </div>
                    </CardContent>
                  </Card>
                ))}
                {((previewData as any).courses || []).length === 0 && (
                  <Card className="col-span-full"><CardContent className="py-8 text-center text-cyan-300">No published courses yet</CardContent></Card>
                )}
              </div>
          ...
          {viewMode === "learner" && previewData && (
            <div className="space-y-6">
              <div className="flex items-center gap-2 text-sm text-black dark:text-foreground bg-emerald-50 p-3 rounded-lg">
              ...
              {(previewData as any).user && (
                <Card>
                  ...
                  <CardContent>
                    <p className="text-sm text-black dark:text-foreground">{(previewData as any).user.email}</p>
                  </CardContent>
                </Card>
              )}
              ...
              <div className="space-y-3">
                    ...
                    <CardContent className="py-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 text-xs text-black dark:text-foreground">
                ...
                {((previewData as any).enrollments || []).length === 0 && (
                  <Card><CardContent className="py-8 text-center text-cyan-300">No enrollments yet</CardContent></Card>
                )}
              </div>
            </div>
          )}
          ...
          {viewMode === "coach" && previewData && (
            <div className="space-y-6">
              <div className="flex items-center gap-2 text-sm text-black dark:text-foreground bg-violet-50 p-3 rounded-lg">
          ...
          {viewMode === "hr" && previewData && (
            <div className="space-y-6">
              <div className="flex items-center gap-2 text-sm text-black dark:text-foreground bg-orange-50 p-3 rounded-lg">
          ...
          {viewMode === "admin" && previewData && (
            <div className="space-y-6">
              <div className="flex items-center gap-2 text-sm text-black dark:text-foreground bg-amber-50 p-3 rounded-lg">
