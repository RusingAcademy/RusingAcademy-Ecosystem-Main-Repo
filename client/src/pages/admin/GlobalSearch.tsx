  ...
  return (
    <>
      ...
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 text-sm text-black dark:text-white bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 rounded-lg transition-colors w-full max-w-sm"
      >
        <Search className="w-4 h-4" />
        <span className="flex-1 text-left">Search everything...</span>
        <kbd className="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.5 text-xs font-mono bg-white dark:bg-slate-900 rounded border text-cyan-300">
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-start justify-center pt-[15vh]">
          <div
            ref={containerRef}
            className="w-full max-w-xl bg-white dark:bg-slate-900 rounded-xl shadow-2xl border overflow-hidden"
          >
            {/* Search Input */}
                  ...
                  {quickActions.map((action: any) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-sm font-medium text-black dark:text-white">{action.label}</span>
                ...
                <div className="p-2">
                  {(results as any[]).length === 0 ? (
                    <div className="py-8 text-center">
                      ...
                      <p className="text-sm text-black dark:text-white">No results for "{query}"</p>
                      ...
                      {(results as any[]).map((r: any, idx: number) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-slate-800 text-black dark:text-white capitalize">
            ...
            <div className="px-4 py-2 border-t bg-gray-50 dark:bg-slate-900 flex items-center justify-between">
              <div className="flex items-center gap-4 text-xs text-cyan-300">
                <span className="flex items-center gap-1">
                  <kbd className="px-1 py-0.5 bg-white dark:bg-slate-900 rounded border text-[10px]">↑↓</kbd> Navigate
                </span>
                <span className="flex items-center gap-1">
                  <kbd className="px-1 py-0.5 bg-white dark:bg-slate-900 rounded border text-[10px]">↵</kbd> Open
                </span>
                <span className="flex items-center gap-1">
                  <kbd className="px-1 py-0.5 bg-white dark:bg-slate-900 rounded border text-[10px]">ESC</kbd> Close
                </span>
              </div>
