...
function LoadingSkeleton() {
  return (
    <div className="animate-pulse space-y-4 max-w-7xl mx-auto">
      ...
      <div className="h-10 bg-gray-100 dark:bg-white/[0.08] dark:backdrop-blur-md dark:border-white/15 rounded-lg" />
      {[1, 2, 3, 4, 5].map(i => <div key={i} className="h-16 bg-gray-100 dark:bg-white/[0.08] dark:backdrop-blur-md dark:border-white/15 rounded-lg" />)}
    </div>
  );
}
      ...
      <div className="max-w-7xl mx-auto">
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-colors ${
                statusFilter === tab.key
                  ...
                  : "bg-white dark:bg-white/[0.08] dark:backdrop-blur-md dark:border-white/15 text-gray-600 border-gray-200 dark:border-white/15 hover:border-blue-600/40"
              }`}
            >
              {tab.label} ({tab.count})
            </button>
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-gray-200 dark:border-white/15 dark:border-white/15 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600"
          />
        </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.map(p => {
                    ...
                    return (
                      <tr key={p.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 dark:bg-white/[0.08] dark:backdrop-blur-md cursor-pointer">
                        ...
                        <td className="py-3 px-2 text-center">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${LEVEL_COLORS[p.currentLevel ?? ""] ?? "bg-gray-50 dark:bg-white/[0.08] dark:backdrop-blur-md text-gray-600"}`}>
                            {p.currentLevel ?? "—"}
                          </span>
                        </td>
                        <td className="py-3 px-2 text-center">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${LEVEL_COLORS[p.targetLevel ?? ""] ?? "bg-gray-50 dark:bg-white/[0.08] dark:backdrop-blur-md text-gray-600"}`}>
                            {p.targetLevel ?? "—"}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center">
