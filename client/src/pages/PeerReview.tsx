      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-8">
            <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-2xl p-4 border border-gray-100 dark:border-white/15 shadow-sm text-center">
          ...
          {/* Tabs */}
          <div className="flex gap-1 mb-6 bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-xl p-1 border border-gray-100 dark:border-white/15 shadow-sm w-fit">
          ...
          {activeTab === "guide" && (
            <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-2xl p-8 border border-gray-100 dark:border-white/15 shadow-sm">
              <h2 className="text-xl font-bold text-gray-900 dark:text-foreground mb-6 flex items-center gap-2">
                <span className="material-icons text-teal-700">school</span>
                Peer Review Guide
              </h2>
              <div className="space-y-6">
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-foreground mb-2">How Peer Review Works</h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    ...
                    <div key={i} className="p-4 rounded-xl bg-gray-50 dark:bg-white/[0.08] dark:backdrop-blur-md border border-gray-100">
                      <span className="material-icons text-teal-700 text-2xl mb-2 block">{item.icon}</span>
                      <h4 className="font-semibold text-gray-900 dark:text-foreground text-sm mb-1">{item.title}</h4>
            ...
            <div>
              {activeReview ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-2xl p-6 border border-gray-100 dark:border-white/15 shadow-sm">
                    <h3 className="font-semibold text-gray-900 dark:text-foreground mb-2">Submission to Review</h3>
                    <div className="p-4 bg-gray-50 dark:bg-white/[0.08] dark:backdrop-blur-md rounded-xl text-sm text-gray-700 dark:text-muted-foreground leading-relaxed font-serif whitespace-pre-wrap">
                      {activeReview.content || "No content available for this submission."}
                    </div>
                  </div>

                  <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-2xl p-6 border border-gray-100 dark:border-white/15 shadow-sm">
                    <h3 className="font-semibold text-gray-900 dark:text-foreground mb-4">Your Evaluation</h3>
                    <div className="space-y-5">
                      <ScoreSlider label="Grammar" value={grammarScore} onChange={setGrammarScore} color="var(--brand-teal, var(--teal))" />
                      <ScoreSlider label="Vocabulary" value={vocabularyScore} onChange={setVocabularyScore} color="var(--color-violet-500, var(--accent-purple))" />
                      <ScoreSlider label="Coherence" value={coherenceScore} onChange={setCoherenceScore} color="var(--semantic-warning, var(--warning))" />
                      <div className="pt-2 border-t border-gray-100 dark:border-white/15 flex items-center justify-between">
                        <span className="text-sm font-semibold text-gray-700">Overall Score</span>
                        <span className="text-lg font-bold" style={{ color: overallScore >= 80 ? "var(--semantic-success, var(--success))" : overallScore >= 60 ? "var(--semantic-warning, var(--warning))" : "var(--semantic-danger, var(--danger))" }}>{overallScore}%</span>
                      </div>
                    </div>
                  </div>

                  <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-2xl p-6 border border-gray-100 dark:border-white/15 shadow-sm space-y-4">
                    <div>
                      <label className="text-sm font-medium text-gray-700 dark:text-muted-foreground mb-1 block">Detailed Feedback *</label>
                      <textarea value={feedback} onChange={e => setFeedback(e.target.value)} rows={4}
                        placeholder="Provide specific, constructive feedback on the writing..."
                        className="w-full p-3 rounded-xl border border-gray-200 dark:border-white/15 dark:border-white/15 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700/30 resize-none" />
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-700 dark:text-muted-foreground mb-1 block">Strengths</label>
                      <textarea value={strengths} onChange={e => setStrengths(e.target.value)} rows={2}
                        placeholder="What did the writer do well?"
                        className="w-full p-3 rounded-xl border border-gray-200 dark:border-white/15 dark:border-white/15 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700/30 resize-none" />
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-700 dark:text-muted-foreground mb-1 block">Areas for Improvement</label>
                ...
                <div className="space-y-3">
                  {loadingPending ? (
                    ...
                    <div key={review.id} className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-xl p-5 border border-gray-100 dark:border-white/15 shadow-sm hover:shadow-md transition-shadow cursor-pointer"
          ...
          {activeTab === "completed" && (
            <div className="space-y-3">
              {!completedReviews?.length ? (
                ...
                <div key={review.id} className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-xl p-5 border border-gray-100 dark:border-white/15 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <div className="font-semibold text-gray-900">Submission #{review.submissionId}</div>
                    <div className="text-lg font-bold" style={{ color: (review.overallScore ?? 0) >= 80 ? "var(--semantic-success, var(--success))" : (review.overallScore ?? 0) >= 60 ? "var(--semantic-warning, var(--warning))" : "var(--semantic-danger, var(--danger))" }}>
                      {review.overallScore}%
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-3">
                    {[
                      { label: "Grammar", value: review.grammarScore, color: "var(--brand-teal, var(--teal))" },
                      { label: "Vocabulary", value: review.vocabularyScore, color: "var(--color-violet-500, var(--accent-purple))" },
                      { label: "Coherence", value: review.coherenceScore, color: "var(--semantic-warning, var(--warning))" },
                    ].map((s, i) => (
