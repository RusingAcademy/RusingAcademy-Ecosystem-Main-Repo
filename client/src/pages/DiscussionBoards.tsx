...
const CATEGORIES = [
  ...
  { id: "vocabulary", label: "Vocabulary", icon: "translate", color: "var(--color-violet-500, var(--accent-purple))" },
  { id: "sle_prep", label: "SLE Prep", icon: "quiz", color: "var(--semantic-warning, var(--warning))" },
  { id: "study_tips", label: "Study Tips", icon: "lightbulb", color: "var(--semantic-success, var(--success))" },
  { id: "resources", label: "Resources", icon: "library_books", color: "var(--semantic-danger, var(--danger))" },
  { id: "general", label: "General", icon: "chat", color: "var(--color-gray-500, var(--muted-foreground))" },
];
    ...
    return (
      <DashboardLayout>
        <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          {thread.isLoading ? (
            <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-20 bg-gray-100 dark:bg-white/[0.08] dark:backdrop-blur-md dark:border-white/15 rounded-xl animate-pulse" />)}</div>
          ) : t ? (
            <>
              <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md border border-gray-200 dark:border-white/15 dark:border-white/15 rounded-xl p-6 shadow-sm mb-4">
                ...
                <h1 className="text-xl font-bold text-gray-900 dark:text-foreground mb-3">{t.title}</h1>
                <p className="text-sm text-gray-700 dark:text-muted-foreground whitespace-pre-wrap leading-relaxed">{t.content}</p>
              ...
              <div className="space-y-3 mb-4">
                {r.map((reply) => (
                  <div key={reply.id} className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md border border-gray-100 dark:border-white/15 rounded-xl p-4 shadow-sm">
                    ...
                    <p className="text-sm text-gray-700 dark:text-muted-foreground whitespace-pre-wrap">{reply.content}</p>
                  </div>
                ))}
                {r.length === 0 && (
              ...
              {user && (
                <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md border border-gray-200 dark:border-white/15 dark:border-white/15 rounded-xl p-4 shadow-sm">
                  <textarea
                    value={replyContent}
                    onChange={(e) => setReplyContent(e.target.value)}
                    placeholder="Write your reply..."
                    className="w-full border border-gray-200 dark:border-white/15 dark:border-white/15 rounded-lg p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-teal-700/30 min-h-[80px]"
                  />
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        {showNewThread && (
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-white/[0.08] dark:backdrop-blur-md rounded-2xl p-6 w-full max-w-lg shadow-xl">
              ...
              <textarea
                ...
                className="w-full border border-gray-200 dark:border-white/15 dark:border-white/15 rounded-lg p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-teal-700/30 min-h-[120px]"
              />
        ...
        {/* Thread List */}
        {threads.isLoading ? (
          <div className="space-y-3">{[1,2,3,4].map(i => <div key={i} className="h-24 bg-gray-100 dark:bg-white/[0.08] dark:backdrop-blur-md dark:border-white/15 rounded-xl animate-pulse" />)}</div>
            ...
            {(threads.data || []).map((t) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left bg-white dark:bg-white/[0.08] dark:backdrop-blur-md border border-gray-200 dark:border-white/15 dark:border-white/15 rounded-xl p-4 shadow-sm hover:shadow-md hover:border-teal-700/30 transition-all"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <h3 className="text-sm font-semibold text-gray-900 dark:text-foreground truncate">{t.title}</h3>
                      </div>
