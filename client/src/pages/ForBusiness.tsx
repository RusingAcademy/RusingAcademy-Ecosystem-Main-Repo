      ...
      <section className="relative overflow-hidden">
        ...
        <div className="container relative pt-10 pb-12 md:pt-14 md:pb-16">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-black dark:text-foreground mb-6 leading-tight">
              {t.title}{" "}
            ...
            <p className="text-lg md:text-xl text-black dark:text-foreground/90 mb-4 md:mb-6 lg:mb-10 max-w-3xl mx-auto leading-relaxed">
              {t.subtitle}
            </p>
            ...
            {t.benefits.map((benefit, i) => (
              <Card key={i} className="border-slate-200 dark:border-teal-800 hover:shadow-lg transition-shadow duration-300">
                <CardContent className="p-6">
                  ...
                  <h3 className="text-lg font-semibold text-black dark:text-foreground mb-2">{benefit.title}</h3>
                  <p className="text-black dark:text-foreground dark:text-cyan-300">{benefit.desc}</p>
                </CardContent>
              </Card>
            ))}
          </div>
            ...
            {t.packages.map((pkg, i) => (
              <Card key={i} className={`relative border-2 ${pkg.popular ? 'border-teal-500 dark:border-teal-400 shadow-xl shadow-teal-500/10' : 'border-slate-200 dark:border-teal-800'} hover:shadow-lg transition-all duration-300`}>
                ...
                <CardHeader className="text-center pb-4">
                  <CardTitle className="text-xl font-bold text-black dark:text-foreground">{pkg.name}</CardTitle>
                  <div className="mt-4">
                    <span className="text-xl md:text-3xl lg:text-4xl font-bold text-teal-600 dark:text-teal-400">{pkg.price}</span>
                    <span className="text-black dark:text-foreground dark:text-cyan-300 ml-2">{pkg.period}</span>
                  </div>
                  <p className="text-sm text-black dark:text-foreground dark:text-cyan-300 mt-2">{pkg.description}</p>
                </CardHeader>
                  ...
                  <ul className="space-y-3 mb-6">
                    {pkg.features.map((feature, j) => (
                      <li key={j} className="flex items-start gap-3">
                        ...
                        <span className="text-black dark:text-foreground/90 text-sm">{feature}</span>
                      </li>
                    ))}
                  </ul>
            ...
            {t.industries.map((industry, i) => (
              <Card key={i} className="border-slate-200 dark:border-teal-800 hover:shadow-lg hover:border-teal-300 dark:hover:border-teal-700 transition-all duration-300 group cursor-pointer">
                <CardContent className="p-6 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-black dark:text-foreground mb-2">{industry.name}</h3>
                  <p className="text-sm text-black dark:text-foreground dark:text-cyan-300">{industry.desc}</p>
                </CardContent>
              </Card>
            ))}
          </div>
              ...
              <Card key={i} className="border-slate-200 dark:border-teal-800 bg-white dark:bg-white/[0.08] dark:backdrop-blur-md dark:border-white/15 dark:bg-white/[0.08] dark:backdrop-blur-md">
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="font-semibold text-black dark:text-foreground">{testimonial.author}</div>
                      <div className="text-sm text-black dark:text-foreground dark:text-cyan-300">{testimonial.role}, {testimonial.company}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
