    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>/</span>
          <span className="text-gray-900 dark:text-foreground font-medium">{program.title}</span>
        </div>

        {/* Program Hero */}
        <div className="relative rounded-2xl overflow-hidden p-8" style={{
          background: isESL
            ? "linear-gradient(135deg, #1a365d 0%, var(--color-blue-600, var(--semantic-info)) 100%)"
            : "linear-gradient(135deg, #7c2d12 0%, var(--semantic-danger, var(--danger)) 100%)",
          minHeight: "160px",
        }}>
          <div className="absolute top-0 right-0 w-48 h-48 rounded-full opacity-10" style={{
            background: "radial-gradient(circle, var(--semantic-warning, var(--warning)), transparent)",
            transform: "translate(20%, -20%)",
          }} />
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-bold px-3 py-1 rounded-full bg-white dark:bg-background/20 text-white backdrop-blur-sm">
                  ...
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="flex-1 p-5">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <h2 className="text-xl font-bold text-gray-900 dark:text-foreground group-hover:text-teal-700 transition-colors" style={{
                        ...
                        {path.modules.map((mod) => {
                          ...
                          return (
                            <span key={mod.id} className="text-[10px] px-2 py-1 rounded-full font-medium" style={{
                              ...
                              color: modPct === 100 ? "var(--semantic-warning, var(--warning))" : "var(--brand-teal, var(--teal))",
                            }}>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1 h-2.5 rounded-full overflow-hidden" style={{ background: "rgba(0,128,144,0.06)" }}>
                          <div className="h-full rounded-full transition-all duration-700" style={{
                            width: `${progressPct}%`,
                            background: isComplete ? "linear-gradient(90deg, var(--semantic-warning, var(--warning)), #ffd700)" : "linear-gradient(90deg, var(--brand-teal, var(--teal)), #00a0b0)",
                          }} />
                        </div>
                        <span className="text-sm font-bold" style={{ color: isComplete ? "var(--semantic-warning, var(--warning))" : "var(--brand-teal, var(--teal))" }}>
                          {progressPct}%
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        {!isLocked && (
                          <span className="text-xs font-semibold flex items-center gap-1 group-hover:gap-2 transition-all" style={{ color: "var(--brand-teal, var(--teal))" }}>
