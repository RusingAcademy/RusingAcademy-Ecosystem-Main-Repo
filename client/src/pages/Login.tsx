/*
 * RusingÂcademy Auth Page — "Institutional Elegance" Portal Design
 * ─────────────────────────────────────────────────────────────────
 * Merged from Learner Portal's premium visual design with Main's
 * complete authentication infrastructure (customAuth + OAuth).
 *
 * Layout: 100vh, no scroll, asymmetric split (55% left / 45% right).
 * Left: Cream gradient with CSS MacBook mockup, logo, stats.
 * Right: Deep teal gradient with glassmorphism auth card, floating orbs.
 * Brand: Foundation (#0F3D3E) · Gold (#D4A853) · Cream (#F5F1D6)
 * Typography: Merriweather (display) + Inter (UI) — design system tokens
 * Auth: customAuth (email/password) + Google OAuth + Microsoft OAuth
 *
 * Migration: Portal design merge — feat/portal-design-merge
 */
...
const LOGO_FULL =
  "https://rusingacademy-cdn.b-cdn.net/images/logos/rusingacademy-official.png";
  ...
  return (
    <div className="flex flex-col gap-[2px]">
      ...
      <div className="flex gap-[2px]">
        <div style={{ ...KEY_STYLE, height: "8px", flex: 1, fontSize: "4px" }}>fn</div>
        <div style={{ ...KEY_STYLE, height: "8px", flex: 1, fontSize: "4px" }}>⌃</div>
        <div style={{ ...KEY_STYLE, height: "8px", flex: 1, fontSize: "4px" }}>⌥</div>
        <div style={{ ...KEY_STYLE, height: "8px", flex: 1.3, fontSize: "4px" }}>⌘</div>
        <div style={{ ...KEY_STYLE, height: "8px", flex: 5, fontSize: "4px" }}> </div>
        <div style={{ ...KEY_STYLE, height: "8px", flex: 1.3, fontSize: "4px" }}>⌘</div>
        <div style={{ ...KEY_STYLE, height: "8px", flex: 1, fontSize: "4px" }}>⌥</div>
...
function MicrosoftIcon({ className }: { className?: string }) {
  return (
    <svg className={className} width="16" height="16" viewBox="0 0 21 21">
...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════
   Combines Portal's "Institutional Elegance" visual design
...
function LoginContent() {
  ...
  const loginMutation = trpc.customAuth.login.useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.success) {
        ...
        setError(
          t("Login failed. Please try again.", "Échec de connexion. Veuillez réessayer.")
        );
  ...
  /* ═══════════════════════════════════════════════════════
     RENDER — Institutional Elegance Layout
     ═══════════════════════════════════════════════════════ */
  return (
    <div className="h-screen w-screen flex flex-col lg:flex-row overflow-hidden">
      ...
      >
        {/* Floating orbs */}
        <FloatingOrb size={200} color="rgba(15,61,62,0.35)" top="-5%" left="60%" delay={0} />
        <FloatingOrb size={160} color="rgba(212,168,83,0.12)" top="70%" left="-10%" delay={2} />
        <FloatingOrb size={120} color="rgba(15,61,62,0.20)" top="40%" left="80%" delay={4} />
          ...
          >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    autoComplete="email"
                    className="w-full pl-10 pr-3 py-2.5 rounded-lg bg-white/[0.06] border border-white/10 text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-[var(--barholex-gold)]/50 focus:bg-white/[0.08] transition-all disabled:opacity-50"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-2.5 rounded-lg bg-white/[0.06] border border-white/10 text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-[var(--barholex-gold)]/50 focus:bg-white/[0.08] transition-all disabled:opacity-50"
                  />
                  <button
                    type="button"
            ...
            <div className="grid grid-cols-2 gap-2.5 mb-4">
              <button
                ...
                disabled={isPending || loginSuccess}
                className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-white text-[#333] text-xs font-semibold shadow-sm hover:shadow-lg hover:bg-gray-50 hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <GoogleIcon />
                Google
              </button>
              <button
                ...
                disabled={isPending || loginSuccess}
                className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-[#2F2F2F] text-white text-xs font-semibold shadow-sm hover:bg-[#3a3a3a] hover:shadow-lg hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <MicrosoftIcon />
                Microsoft
              </button>
              ...
              <div className="flex items-center gap-1.5">
                {[
                  ...
                  <a
                    ...
                    className="w-7 h-7 rounded-full flex items-center justify-center text-[var(--barholex-gold)]/50 hover:text-[var(--barholex-gold)] hover:bg-[var(--barholex-gold)]/10 transition-all"
                  >
        ...
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 mb-5">
            <div className="relative">
              ...
              <input
                ...
                disabled={isPending || loginSuccess}
                autoComplete="email"
                className="w-full pl-10 pr-3 py-3 rounded-xl bg-white/[0.06] border border-white/10 text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-[var(--barholex-gold)]/50 transition-all disabled:opacity-50"
              />
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30" />
              <input
                ...
                disabled={isPending || loginSuccess}
                autoComplete="current-password"
                className="w-full pl-10 pr-10 py-3 rounded-xl bg-white/[0.06] border border-white/10 text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-[var(--barholex-gold)]/50 transition-all disabled:opacity-50"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
            ...
            <button
              type="submit"
              disabled={isPending || loginSuccess}
              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-white text-sm font-bold shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              style={{
                background: isPending
                  ? "linear-gradient(135deg, #1a4a4a 0%, #2a6666 50%, #3a8080 100%)"
                  : loginSuccess
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            <button
              ...
              onClick={handleGoogleSignIn}
              disabled={isPending || loginSuccess}
              className="flex items-center justify-center gap-2 py-2.5 rounded-xl bg-white text-[#333] text-xs font-semibold disabled:opacity-50"
            >
              <GoogleIcon />
              Google
            </button>
            <button
              ...
              onClick={handleMicrosoftSignIn}
              disabled={isPending || loginSuccess}
              className="flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[#2F2F2F] text-white text-xs font-semibold disabled:opacity-50"
            >
              <MicrosoftIcon />
              Microsoft
            </button>
