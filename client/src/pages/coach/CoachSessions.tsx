      ...
      <div className="grid grid-cols-7 gap-0 text-center text-xs">
        {days.map((day, i) => (
          <div key={i} className={`py-1.5 rounded-full relative cursor-pointer ${
            ...
            day ? "text-gray-700 dark:text-muted-foreground hover:bg-gray-100" : ""
          }`}>
            {day || ""}
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
            ...
            <div className="space-y-2">
              {pendingSessions.map((s: any) => (
                <div key={s.id} className="flex items-center justify-between bg-white dark:bg-background rounded-lg p-3 border border-amber-100">
                  <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => declineMutation.mutate({ sessionId: s.id, reason: "Declined by coach" })}
                      className="text-xs px-3 py-1.5 bg-gray-100 dark:bg-card text-gray-600 rounded-lg hover:bg-gray-200 transition-colors">
          ...
          <div className="lg:col-span-2">
            ...
            {isLoading ? (
              <div className="space-y-3">
                {[1, 2, 3, 4].map((i) => <div key={i} className="h-20 bg-gray-50 dark:bg-background rounded-xl animate-pulse" />)}
              </div>
            ) : filtered.length === 0 ? (
              <div className="text-center py-6 md:py-8 lg:py-12 bg-white dark:bg-background rounded-xl border border-gray-100">
              ...
              <div className="space-y-3">
                {filtered.map((session: any) => {
                  ...
                  return (
                    <div key={session.id} className="bg-white dark:bg-background rounded-xl border border-gray-100 dark:border-border p-4 hover:shadow-md transition-shadow flex items-center gap-4">
