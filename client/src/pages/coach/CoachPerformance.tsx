  ...
  return (
    <div className="flex flex-col items-center">
      <svg width="100" height="100" className="transform -rotate-90" role="img" aria-label={`${label}: ${value}%`}>
        <circle cx="50" cy="50" r="40" fill="none" stroke="var(--admin-table-header-bg, #f3f4f6)" strokeWidth="8" />
        {!loading && (
      ...
      <div className="max-w-7xl mx-auto" role="main" aria-label={lang === "fr" ? "Performance du coach" : "Coach Performance"}>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-100 p-6 mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
            <MetricRing value={satisfactionPct} label={lang === "fr" ? "Satisfaction" : "Satisfaction"} color="var(--semantic-success, #059669)" loading={isLoading} />
            <MetricRing value={successRate || satisfactionPct} label={lang === "fr" ? "Taux de réussite" : "Success Rate"} color={ACCENT} loading={isLoading} />
            <MetricRing value={totalStudents > 0 ? Math.min(Math.round((totalStudents / (totalStudents + 2)) * 100), 100) : 0} label={lang === "fr" ? "Rétention" : "Retention"} color="var(--color-blue-600, #2563eb)" loading={isLoading} />
            <MetricRing value={avgRating > 0 ? Math.round(avgRating * 20) : 0} label={lang === "fr" ? "Note globale" : "Overall Rating"} color="var(--semantic-warning, #d97706)" loading={isLoading} />
          </div>
        </div>
                ...
                {learners.slice(0, 6).map((learner: any, i: number) => {
                  ...
                  return (
                    <div key={learner.id || i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-600 to-[var(--color-purple-600, #9333ea)] flex items-center justify-center text-white font-bold text-xs">
                        {name.charAt(0)}
                      </div>
