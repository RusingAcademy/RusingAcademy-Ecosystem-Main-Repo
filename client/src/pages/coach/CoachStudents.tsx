      ...
      <div className="max-w-7xl mx-auto" role="main" aria-label={lang === "fr" ? "Mes étudiants" : "My Students"}>
          ...
          <div className="flex gap-2">
            <div className="relative">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="pl-9 pr-4 py-2 text-sm border border-gray-200 dark:border-slate-700 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-[var(--color-violet-600, var(--color-violet-600, #7c3aed))]/20 focus:border-violet-600 outline-none w-48" />
            </div>
          </div>
        </div>
        ...
        {learnersQuery.isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="h-48 bg-gray-50 dark:bg-slate-900 rounded-xl animate-pulse" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map((student: any) => {
              ...
              return (
                <div key={student.id || student.learnerId} className="bg-white dark:bg-slate-800 dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-700 p-5 hover:shadow-md transition-shadow group">
