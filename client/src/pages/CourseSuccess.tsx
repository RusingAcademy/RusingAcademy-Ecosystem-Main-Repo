...
{/* Success Hero */}
      <section className="py-16 md:py-24">
        <div className="container px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-xl text-black dark:text-white dark:text-white mb-6">
              {l.subtitle}
            </p>
            
            {/* Email Confirmation Notice */}
            {user?.email && (
              <div className="flex items-center justify-center gap-2 text-sm text-black dark:text-white dark:text-white mb-8">
                <Mail className="w-4 h-4" />
                <span>
                  {l.emailSent} <strong className="text-black dark:text-white dark:text-white">{user.email}</strong>
                </span>
              </div>
            )}
          </motion.div>
      ...
      {/* Next Steps Section */}
      <section className="py-12 bg-white dark:bg-slate-800 dark:bg-slate-900">
        <div className="container px-4 sm:px-6 lg:px-8">
                ...
                <Card className={`h-full ${index === 0 ? "border-teal-300 bg-teal-50/50" : ""}`}>
                  <CardContent className="p-6">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <h3 className="font-semibold text-black dark:text-white dark:text-white mb-2">{step.title}</h3>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {l.features.map((feature, index) => (
              <motion.div
                ...
                className="flex items-center gap-3 p-4 bg-white dark:bg-slate-800 dark:bg-slate-900 rounded-lg border border-slate-200 shadow-sm"
              >
                <div className="p-2 bg-amber-100 rounded-lg shrink-0">
                  <feature.icon className="w-5 h-5 text-amber-600" />
                </div>
                <span className="text-black dark:text-white dark:text-white">{feature.text}</span>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="max-w-3xl mx-auto p-6 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-black dark:text-white dark:text-white">{l.guaranteeText}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 bg-gradient-to-r from-teal-600 to-teal-700">
        <div className="container px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/app">
              <Button size="lg" variant="secondary" className="bg-white dark:bg-slate-800 dark:bg-slate-900 text-teal-700 hover:bg-teal-50">
