    ...
    if (showConfetti) {
      ...
      const frame = () => {
        confetti({
          ...
          spread: 55,
          origin: { x: 0 },
          colors: ["var(--semantic-warning, var(--warning))", "#f97316", "var(--semantic-success, var(--success))", "var(--teal)"],
        });
        confetti({
          particleCount: 3,
          angle: 120,
          ...
          >
            ...
            {user?.email && (
              <div className="flex items-center justify-center gap-2 text-sm text-black dark:text-foreground mb-8">
                ...
                <span>
                  {l.emailSent} <strong className="text-black dark:text-foreground">{user.email}</strong>
                </span>
              </div>
            )}
          </motion.div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {l.features.map((feature, index) => (
              ...
              >
                ...
                <span className="text-black dark:text-foreground">{feature.text}</span>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="max-w-3xl mx-auto p-6 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-black dark:text-foreground">{l.guaranteeText}</p>
              </div>
            </div>
          </div>
        </div>
