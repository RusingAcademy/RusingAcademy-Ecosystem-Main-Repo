...
const quickStats = [
  ...
  { label: "Sessions complétées", value: "12", icon: Video, color: "text-violet-600", bg: "bg-violet-50" },
  { label: "Documents lus", value: "8", icon: FileText, color: "text-emerald-600", bg: "bg-emerald-50" },
  { label: "Série en cours", value: "5 jours", icon: Zap, color: "text-amber-600", bg: "bg-amber-50" },
];
  ...
  return (
    <PortalLayout>
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Welcome Section */}
        <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-violet-700 rounded-2xl p-6 text-white relative overflow-hidden">
                ...
                {upcomingSessions.map((session) => (
                  <div key={session.id} className={`p-4 rounded-xl border-2 transition-all ${session.isLive ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-white hover:border-slate-300"}`}>
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-violet-500 to-violet-600 flex items-center justify-center text-white font-semibold">
                ...
                {recentCapsules.map((capsule) => (
                  <div key={capsule.id} className="p-3 rounded-lg border border-slate-200 hover:border-blue-200 hover:bg-blue-50/50 transition-all cursor-pointer group">
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${capsule.type === "video" ? "bg-violet-100 text-violet-600" : capsule.type === "document" ? "bg-blue-100 text-blue-600" : "bg-emerald-100 text-emerald-600"}`}>
              ...
              <CardContent>
                <div className="flex items-center gap-4">
                  <div className="flex -space-x-2">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="w-10 h-10 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 border-2 border-white flex items-center justify-center">
