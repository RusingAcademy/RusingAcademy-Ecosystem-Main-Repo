      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-black mt-1">Vos sessions de coaching personnalisé avec nos experts linguistiques</p>
          </div>
          ...
          <Card className="border-slate-200">
            <CardContent className="pt-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-black">{completedSessions}</p>
                  <p className="text-xs text-black">Sessions complétées</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-black">Sessions à venir</p>
                </div>
              </div>
            </CardContent>
          </Card>
          <Card className="border-slate-200">
            <CardContent className="pt-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-black">4.5h</p>
                  <p className="text-xs text-black">Heures de coaching</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-black">Satisfaction moyenne</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-semibold text-black">Session en cours</h3>
                        ...
                        <p className="text-sm text-black">{upcomingSessions.find(s => s.status === "live")?.title}</p>
                      </div>
                    </div>
            ...
            <div className="flex items-center gap-2 border-b border-slate-200">
              <button onClick={() => setActiveTab("upcoming")} className={cn("px-4 py-2 text-sm font-medium border-b-2 transition-colors", activeTab === "upcoming" ? "border-[#0F3D3E] text-[#0F3D3E]" : "border-transparent text-black hover:text-black")}>
                À venir ({upcomingSessions.length})
              </button>
              <button onClick={() => setActiveTab("past")} className={cn("px-4 py-2 text-sm font-medium border-b-2 transition-colors", activeTab === "past" ? "border-[#0F3D3E] text-[#0F3D3E]" : "border-transparent text-black hover:text-black")}>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {session.status === "completed" && (
                            <span className="flex items-center gap-1 px-2 py-0.5 bg-slate-200 text-black text-xs font-medium rounded-full">
                              <CheckCircle2 className="h-3 w-3" />
                              Terminée
                            </span>
                          )}
                        </div>
                        <p className="text-sm text-black mb-2">{session.description}</p>
                        ...
                        {session.notes && <div className="mt-3 p-3 bg-slate-100 rounded-lg"><p className="text-sm text-black"><strong>Notes:</strong> {session.notes}</p></div>}
          ...
          <div className="space-y-6">
            <Card className="border-slate-200 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-semibold text-black">Mon Coach</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-center">
                  ...
                  <h3 className="font-semibold text-black">{myCoach.name}</h3>
                  <p className="text-sm text-black mb-3">{myCoach.title}</p>
                  <div className="flex items-center justify-center gap-1 mb-4">
                    <Star className="h-4 w-4 text-amber-500 fill-amber-500" />
                    <span className="text-sm font-medium text-black">{myCoach.rating}</span>
                    <span className="text-sm text-black">({myCoach.sessionsCompleted} sessions)</span>
                  </div>
            ...
            <Card className="border-slate-200 shadow-sm bg-gradient-to-br from-[#0F3D3E]-50 to-blue-50">
              <CardContent className="pt-4">
                <div className="text-center">
                  ...
                  <h3 className="font-semibold text-black mb-1">Besoin d'aide?</h3>
                  <p className="text-sm text-black mb-4">Réservez une session supplémentaire avec votre coach</p>
