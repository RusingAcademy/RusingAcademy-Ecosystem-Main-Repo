    ...
    <PortalLayout>
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-3xl font-bold text-black">Rapport de Performance</h1><p className="text-black mt-1">Document officiel pour justification ministérielle</p></div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 mb-6">
          <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-200">
            ...
            <div><h2 className="text-xl font-semibold text-black">{reportData.learnerName}</h2><p className="text-black">Rapport généré le {reportData.generatedDate}</p></div>
          </div>

          <div className="grid grid-cols-2 gap-6 mb-8">
            <div className="bg-white rounded-lg p-4"><div className="flex items-center gap-2 text-black mb-2"><Target className="w-4 h-4" /><span className="text-sm">Objectif</span></div><p className="text-2xl font-bold text-black">Niveau {reportData.currentLevel} → {reportData.targetLevel}</p></div>
            <div className="bg-white rounded-lg p-4"><div className="flex items-center gap-2 text-black mb-2"><TrendingUp className="w-4 h-4" /><span className="text-sm">Progression globale</span></div><p className="text-2xl font-bold text-blue-600">{reportData.overallProgress}%</p></div>
          </div>

          <h3 className="text-lg font-semibold text-black mb-4">Détail des compétences</h3>
          <div className="space-y-4 mb-8">
            {[{label:'Expression orale',value:reportData.oralProgress},{label:'Expression écrite',value:reportData.writtenProgress},{label:'Grammaire',value:reportData.grammarProgress}].map(skill => (
              <div key={skill.label}><div className="flex justify-between mb-1"><span className="text-black">{skill.label}</span><span className="font-medium text-black">{skill.value}%</span></div><div className="h-3 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-blue-600 rounded-full transition-all" style={{width:`${skill.value}%`}} /></div></div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            {[{icon:Award,label:'Tests complétés',value:reportData.testsCompleted},{icon:TrendingUp,label:'Score moyen',value:`${reportData.averageScore}%`},{icon:FileText,label:'Chapitres',value:reportData.chaptersCompleted},{icon:Calendar,label:'Heures d\'étude',value:reportData.studyHours}].map(stat => (
              <div key={stat.label} className="text-center p-4 bg-white rounded-lg"><stat.icon className="w-6 h-6 text-blue-600 mx-auto mb-2" /><p className="text-2xl font-bold text-black">{stat.value}</p><p className="text-sm text-black">{stat.label}</p></div>
            ))}
          </div>
        </div>

        <p className="text-center text-sm text-black">Ce rapport est généré automatiquement par RusingÂcademy et peut être utilisé comme justificatif officiel de formation linguistique.</p>
      </div>
    </PortalLayout>
  );
}
