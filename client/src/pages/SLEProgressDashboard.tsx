...
const labels = {
  en: { title: "S L E Progress Dashboard", description: "Manage and configure s l e progress dashboard" },
  fr: { title: "S L E Progress Dashboard", description: "Gérer et configurer s l e progress dashboard" },
};
...
// ─── Level Colors ────────────────────────────────────────────────────
const levelConfig = {
  X: { bg: "bg-teal-800/20", text: "text-cyan-300", border: "border-slate-500/30", label: "Not Assessed" },
...
export default function SLEProgressDashboard() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-teal-950 via-obsidian to-teal-950 text-white dark-page flex items-center justify-center p-6">
        ...
        >
          ...
          <p className="text-cyan-300 mb-8">{l.noSessionsDesc}</p>
  ...
  const trendColor = summary.recentTrend === "improving" ? "text-emerald-400" : summary.recentTrend === "declining" ? "text-red-400" : "text-cyan-300";
    ...
    <div className="min-h-screen bg-gradient-to-b from-teal-950 via-obsidian to-teal-950 text-white dark-page">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-cyan-300">{l.subtitle}</p>
        </motion.div>
        ...
        >
          ...
          {/* Total Sessions */}
          <div className="p-5 rounded-xl bg-foundation/50 border border-teal-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <motion.div
            ...
            className="p-6 rounded-xl bg-foundation/50 border border-teal-800"
          >
            <h3 className="font-semibold text-white mb-1">{l.criteriaTitle}</h3>
            <p className="text-xs text-cyan-300 mb-4">{l.criteriaSubtitle}</p>
            <div className="h-64">
          ...
          >
            ...
            <p className="text-xs text-cyan-300 mb-4">{l.trendSubtitle}</p>
            <div className="h-64">
            ...
            {prediction && (
              <div className="mt-4 p-4 rounded-lg bg-gradient-to-r from-violet-500/10 to-blue-500/10 border border-violet-500/20">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={cn(
                        "text-2xl font-black",
                        levelConfig[prediction.level as keyof typeof levelConfig]?.text || "text-cyan-300"
                      )}>
                        {prediction.level}
                      </span>
                ...
                <p className="text-[10px] text-white/90 mt-2">{l.predictionDesc}</p>
              </div>
            )}
          </motion.div>
        </div>
        ...
        <motion.div
          ...
          className="rounded-xl bg-foundation/50 border border-teal-800 overflow-hidden"
        >
          <div className="px-6 py-4 border-b border-teal-800">
            <h3 className="font-semibold text-white">{l.historyTitle}</h3>
            <p className="text-xs text-cyan-300">{l.historySubtitle}</p>
          </div>
          ...
          {history && history.sessions.length > 0 ? (
            <div className="divide-y divide-slate-700/50">
              {history.sessions.map((session) => {
                ...
                const statusColor = session.status === "completed" ? "text-emerald-400" : session.status === "active" ? "text-blue-400" : "text-white/90";
                ...
                return (
                  <div key={session.id}>
                    ...
                    >
                      ...
                      {/* Expand */}
                      {isExpanded ? (
                        <ChevronUp className="h-4 w-4 text-white/90 flex-shrink-0" />
                      ) : (
                        <ChevronDown className="h-4 w-4 text-white/90 flex-shrink-0" />
                      )}
                    </button>
          ...
          {history && history.total > (historyPage + 1) * 10 && (
            <div className="px-6 py-4 border-t border-teal-800 text-center">
              <Button
                variant="ghost"
                size="sm"
