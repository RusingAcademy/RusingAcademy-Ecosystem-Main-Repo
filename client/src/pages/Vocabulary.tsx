        ...
        <div className="max-w-6xl mx-auto p-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div>
              <div className="flex items-center gap-3">
                {viewMode === "quiz" && (
                  <button onClick={() => setViewMode("list")} className="p-1.5 rounded-lg hover:bg-gray-100 dark:bg-card focus:outline-none focus:ring-2 focus:ring-teal-700/30" aria-label={t("flashcards.backToDecks")}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6" role="region" aria-label={t("grammar.stats")}>
              {Object.entries(MASTERY_CONFIG).map(([key, config]) => (
                <button key={key} onClick={() => setFilterMastery(filterMastery === key ? null : key)}
                  className={`bg-white dark:bg-card rounded-xl border p-4 text-center transition-all hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-teal-700/30 ${filterMastery === key ? "border-teal-700 ring-1 ring-teal-700/20" : "border-gray-100"}`}
              ...
              <div className="bg-white dark:bg-background rounded-xl border border-gray-100 dark:border-border p-4 text-center" role="status">
            ...
            <>
              ...
              {isLoading ? (
                <div className="space-y-3" role="status" aria-label={t("skillLabs.loading")}>
                  {[1,2,3,4,5].map(i => <div key={i} className="h-20 rounded-xl bg-gray-100 dark:bg-card animate-pulse" />)}
                  <span className="sr-only">{t("skillLabs.loading")}</span>
                </div>
                ...
                <div className="space-y-2" role="list" aria-label={t("vocab.title")}>
                  {filteredItems.map(item => {
                    ...
                    return (
                      <div key={item.id} role="listitem" className="bg-white dark:bg-background rounded-xl border border-gray-100 dark:border-border p-4 group hover:shadow-sm transition-all">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-1 flex-wrap">
                              ...
                              {item.partOfSpeech && (
                                <span className="px-2 py-0.5 rounded-full bg-gray-100 dark:bg-card text-[10px] text-gray-500 font-medium italic">{item.partOfSpeech}</span>
                              )}
                            ...
                            <div className="mt-3">
                              ...
                              <div className="h-1.5 bg-gray-100 dark:bg-card rounded-full overflow-hidden" role="progressbar"
                          ...
                          <button onClick={() => { if (confirm(t("flashcards.delete") + "?")) deleteItem.mutate({ itemId: item.id }); }}
                            className="p-1.5 rounded-lg hover:bg-gray-100 dark:bg-card opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity shrink-0 focus:outline-none focus:ring-2 focus:ring-red-300"
            ...
            quizDone || quizItems.length === 0 ? (
              <div className="text-center py-16 max-w-md mx-auto" role="status">
                ...
                <div className="flex gap-3 justify-center">
                  <button onClick={() => setViewMode("list")} className="px-4 py-2 rounded-xl text-sm font-medium text-gray-600 border border-gray-200 dark:border-border dark:border-border hover:bg-gray-50 dark:bg-background focus:outline-none focus:ring-2 focus:ring-gray-300">
                  ...
                  {!quizRevealed ? (
                    <>
                      ...
                      <input id="quiz-answer" type="text" placeholder={isFr ? "Tapez votre réponse..." : "Type your answer..."} value={quizAnswer}
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-border dark:border-border text-center text-gray-900 dark:text-foreground text-base mb-4 focus:outline-none focus:ring-2 focus:ring-teal-700/20"
                        autoFocus />
                    ...
                    <>
                      <div className="p-4 rounded-xl bg-gray-50 dark:bg-background mb-4">
          ...
          {showForm && (
            <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label={t("vocab.addWord")} onClick={resetForm}>
              <div className="bg-white dark:bg-background rounded-2xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input id="vocab-word" type="text" placeholder={isFr ? "ex. néanmoins" : "e.g., néanmoins"} value={word} onChange={e => setWord(e.target.value)}
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-border dark:border-border text-gray-900 dark:text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-teal-700/20 focus:border-teal-700"
                  ...
                  <div>
                    ...
                    <select id="vocab-pos" value={partOfSpeech} onChange={e => setPartOfSpeech(e.target.value)}
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-border dark:border-border text-gray-900 dark:text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-teal-700/20">
                  ...
                  <div>
                    ...
                    <textarea id="vocab-def" placeholder={isFr ? "Définition optionnelle..." : "Optional definition..."} value={definition} onChange={e => setDefinition(e.target.value)} rows={2}
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-border dark:border-border text-gray-900 dark:text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-teal-700/20 resize-none" />
                  </div>
                  <div>
                ...
                <div className="flex justify-end gap-3 mt-4">
                  <button onClick={resetForm} className="px-4 py-2 rounded-xl text-sm text-gray-600 hover:bg-gray-100 dark:bg-card focus:outline-none focus:ring-2 focus:ring-gray-300">{t("flashcards.cancel")}</button>
