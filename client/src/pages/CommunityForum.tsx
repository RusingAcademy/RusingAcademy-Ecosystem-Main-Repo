  ...
  return (
    <DashboardLayout>
      <div className="max-w-[1100px] space-y-5">
        {/* Header */}
        <div className="bg-white dark:bg-background border border-gray-200 dark:border-border dark:border-border rounded-xl overflow-hidden shadow-sm">
          ...
          {/* Stats Bar */}
          <div className="flex items-center justify-between px-6 py-3 border-t border-gray-100 dark:border-border bg-gray-50">
        ...
        {/* Tab Navigation */}
        <div className="flex gap-1 bg-white dark:bg-background border border-gray-200 dark:border-border dark:border-border rounded-xl p-1 shadow-sm">
          {([
        ...
        {activeTab === "categories" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {forumCategories.map((cat, i) => (
              <div key={i} className="bg-white dark:bg-background border border-gray-200 dark:border-border dark:border-border rounded-xl p-5 shadow-sm hover:shadow-md hover:border-teal-700/20 transition-all cursor-pointer group">
        ...
        {activeTab === "groups" && (
          <div className="space-y-3">
            {studyGroups.map((group, i) => (
              <div key={i} className="bg-white dark:bg-background border border-gray-200 dark:border-border dark:border-border rounded-xl p-5 shadow-sm flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-card text-gray-500 font-medium">{group.language}</span>
                      {group.active && (
            ...
            <button onClick={() => toast.info(lang === "fr" ? "Bientôt disponible" : "Coming soon")}
              className="w-full py-3 rounded-xl border-2 border-dashed border-gray-200 dark:border-border dark:border-border text-sm font-medium text-gray-400 hover:text-teal-700 hover:border-teal-700/30 transition-all flex items-center justify-center gap-2">
              <span className="material-icons" >add</span>
        ...
        {activeTab === "recent" && (
          <div className="space-y-3">
            {recentPosts.map((post, i) => (
              <div key={i} className="bg-white dark:bg-background border border-gray-200 dark:border-border dark:border-border rounded-xl p-5 shadow-sm hover:shadow-md transition-all cursor-pointer group">
                <div className="flex items-start gap-4">
